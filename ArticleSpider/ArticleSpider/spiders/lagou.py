...
class LagouSpider(CrawlSpider):
    ...
    def start_requests(self):
        ...
        cookies = None
        cookies_path = BASE_DIR + "/cookies/lagou.cookie"
        if os.path.exists(cookies_path):
            cookies = pickle.load(open(file=cookies_path, mode='rb'))
        ...
        if not cookies:
            browser.get("https://passport.lagou.com/login/login.html")
            browser.find_element_by_css_selector(".form_body .input.input_white").send_keys("账号")
            browser.find_element_by_css_selector('.form_body input[type="password"]').send_keys("密码")
